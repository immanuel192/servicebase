...
/**
 * Sequelize postgresql adapter
 * Copyright Trung Dang (trungdt@absoft.vn) - 2016
 */

/**
 * TODO: later we will refactor this package, which will split all adapters into packages
 */
    ...
    */
    ...
    instance.registerSchema = function registerSchema(tableSchema) {
        ...
        if (!schemaExisted && tableSchema != '') {
            // create schema
            try {
                // TODO: From Postgresql 9.4 and above, there is an issue with Create Schema command. Need to investigate
 ...
 */
function postgresAdaperInit(options, modelsManager) {
    let sequelize = new Sequelize(options.database, options.user, options.password, options.options)
 ...
 */
module.exports = function exportPostgresDialect(listAdapters, logger) {
    listAdapters[ADAPTER_NAME] = postgresAdaperInit
    log = logger
}